...
function Slideshow(props) {
  ...
  function scrollCarousel(targetID, direction) {
    ...
    const imageWidth = carouselTarget.firstChild.offsetWidth;
    const currentScroll = carouselTarget.scrollLeft;
    const scrollAmount = direction === 'backward' ? -imageWidth : imageWidth;
  ...
  function scrollThumbnail(targetID) {
    ...
    const imageParent = imageTarget.parentElement;
    const horizontalScroll = imageTarget.offsetLeft;

    // Attempts: 
    // 1. (containerWidth - imageWidth) / 2
    // 2. OffsetLeft - (Width - ImageWidth - (Gap * 2)) / 2
    const containerWidth = imageParent.offsetWidth;
    const imageWidth = imageTarget.offsetWidth;
    ...
    imageTarget.parentElement.scrollTo({
      left: horizontalScroll - ((containerWidth - imageWidth) / 2),
      top: 0,
      behavior: 'smooth'
    });
  }
    ...
    <article className={`slides__slideshow slides__slideshow--${category}`}>
      ...
      <main className="slideshow__main" id={`slideshow-main-${category}`}>
        {imageList[0].map((url, index) => (
          <img src={url} alt={imageList[2][index]} id={`${category}${index}`} key={`image-${index}`} loading={index < 2 ? 'eager' : 'lazy'} className="slideshow__main__image" />
        ))}
      </main>
      ...
      <button type='button' className='slideshow__button slideshow__button--previous' onClick={event => scrollCarousel(`slideshow-main-${category}`, 'backward')}>
        <span className="a11y-hidden">Imagem anterior</span>
      </button>

      <button type='button' className='slideshow__button slideshow__button--next' onClick={event => scrollCarousel(`slideshow-main-${category}`, 'forward')}>
        <span className="a11y-hidden">Imagem posterior</span>
      </button>

      <footer className="slideshow__footer">
        {imageList[1].map((url, index) => (
          <button type='button' onClick={event => scrollThumbnail(`${category}${index}`)} style={{backgroundImage: `url(${imageList[1][index]})`}} key={`thumb-${index}`} className="slideshow__footer__thumbnails">
