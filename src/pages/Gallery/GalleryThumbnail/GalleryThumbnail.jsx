      ...
      <section className='thumbnails__category' key={`image-category-${categoryIndex}`}>
        {
          category.map((thumbnail, thumbnailIndex) => {
            ...
            return (
              <button className='category__thumb' onClick={() => clickThumbnails(viewList[categoryIndex][thumbnailIndex], sourceList[categoryIndex][thumbnailIndex], altList[categoryIndex][thumbnailIndex])} style={thumbnailBG} key={`image-thumbnail-${categoryIndex}-${thumbnailIndex}}`}>
                <span className="a11y-hidden">{`Trocar imagem. Descrição: ${altList[categoryIndex][thumbnailIndex]}`}</span>
              </button>
            )
          })
        }
